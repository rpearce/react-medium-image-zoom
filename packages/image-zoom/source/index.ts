...
export interface Detach {
  (targetEl: HTMLImageElement | SVGElement, skipTracking?: boolean): void
}
...
export interface Zoom {
  (targetEl: HTMLImageElement | SVGElement): void
}
...
const { ANIMATING, UNZOOMED, ZOOMED } = State
...
const imageZoom: ImageZoom = ({
  ...
  // setup overlay
  let overlayEl = createElement(DIV) as HTMLDivElement
  setAttribute(
    STYLE,
    stylePosFixed +
      styleAllDirsZero +
  ...
  const detach: Detach = (targetEl, skipTracking = false) => {
    if (!targetEl) return

    setStyleProp(CURSOR, '', targetEl)

    if (!skipTracking) {
      trackedEls = trackedEls.filter(x => x !== targetEl)
    }
  ...
  const update: Update = (opts = {}) => {
    auto = opts.auto ?? auto
    closeText = opts.closeText ?? closeText
    margin = opts.margin ?? margin
    modalText = opts.modalText ?? modalText
    onChange = opts.onChange ?? onChange
    openText = opts.openText ?? openText
  ...
  const zoom: Zoom = (targetEl) => {
    if (!targetEl) return

    const img = cloneElement(true, targetEl)
  ...
  const handleImgClick = (e: Event) => {
    zoom(e.currentTarget as HTMLImageElement)
  }
