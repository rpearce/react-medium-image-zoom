...
describe('getStyleGhost', () => {
  ...
  describe('SVG element with parent', () => {
    it('returns height, width from SVG rect and left/top as SVG offset within parent', () => {
      const parent = document.createElement('div')
      const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg')
      parent.appendChild(svg)

      // SVG is inset 20px from the left and top of its parent
      svg.getBoundingClientRect = () => ({
        top: 70,
        left: 80,
        width: 200,
      ...
      expect(result).toEqual({
        height: 150,
        left: 80 - 60, // svgLeft - parentLeft = 20
        top: 70 - 50, // svgTop - parentTop = 20
        width: 200,
      })
    })
  })
